package com.itlsco.pocs.organization.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "Organization-ear.ear");

        // Current directory is the root of the "Organization-web" project. Go up one level
        // and enter the "Organization-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../Organization-ejb/target/Organization-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "Organization-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/Organization-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Organization-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.itlsco.pocs.organization.test");
